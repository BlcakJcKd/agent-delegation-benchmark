...
def report(root: Path | None = None) -> Path:
    ...
    fieldnames = ["model", "reasoning", "task", "status", "baseline_score", "baseline_check_vector", "final_score", "final_check_vector", "delta_score", "normalized_improvement", "full_pass", "evaluator_tampering", "prohibited_changed_files", "wall_seconds", "input_tokens", "output_tokens", "cache_read_tokens", "reasoning_tokens"]
    ...
    with matrix.open("w", newline="", encoding="utf-8") as handle:
        ...
        for row in rows:
            writer.writerow({"model": row["resolved"]["provider_model_id"], "reasoning": row["resolved"]["reasoning"], "task": row["task"]["family"], "status": row["status"], "baseline_score": row["baseline_score"], "baseline_check_vector": "".join("P" if value else "F" for value in row["baseline_check_vector"]), "final_score": row["final_score"], "final_check_vector": "".join("P" if value else "F" for value in row["final_check_vector"]) if row["final_check_vector"] is not None else None, "delta_score": row["delta_score"], "normalized_improvement": row["normalized_improvement"], "full_pass": row["full_pass"], "evaluator_tampering": row["evaluator_tampering"], "prohibited_changed_files": json.dumps(row["prohibited_changed_files"]), "wall_seconds": row["wall_seconds"], "input_tokens": row["input_tokens"], "output_tokens": row["output_tokens"], "cache_read_tokens": row["cache_read_tokens"], "reasoning_tokens": row["reasoning_tokens"]})
    ...
    for key, group in sorted(grouped.items()):
        ...
        summary.append({"model": key[0], "reasoning": key[1], "mean_baseline": statistics.mean(row["baseline_score"] for row in group), "mean_final": statistics.mean(final_scores) if final_scores else None, "mean_delta": statistics.mean(deltas) if deltas else None, "median_final": statistics.median(final_scores) if final_scores else None, "full_solves": sum(row["full_pass"] is True for row in completed), "attempted": len(group), "completed": len(completed), "completion_rate": len(completed) / len(group), "mean_wall": statistics.mean(row["wall_seconds"] for row in group), "tokens": sum((row["input_tokens"] or 0) + (row["output_tokens"] or 0) for row in group)})
    lines = [f"# {SUITE_NAME}", "", "Class: `public_characterization`; baseline-aware, objective, reproducible, non-adversarially isolated.", "", "| Model | Reasoning | Mean baseline | Mean final | Mean delta | Median final | Full solves | Completed/attempted | Completion rate | Mean wall | Observed input+output |", "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|"]
    for item in summary: lines.append(f"| {item['model']} | {item['reasoning']} | {item['mean_baseline']} | {item['mean_final']} | {item['mean_delta']} | {item['median_final']} | {item['full_solves']} | {item['completed']}/{item['attempted']} | {item['completion_rate']} | {item['mean_wall']} | {item['tokens']} |")
    lines += ["", "Quality is conditional on completed controller-scored attempts. Infrastructure failures and evaluator tampering remain separately reported.", "", "Tool telemetry: unavailable; request metric semantics: harness_session; tokens: AGY-reported usage, not verified provider billing."]
    ...
    audit = ["# Public Characterization V2 audit", "", f"Pilot discrimination observed: `{str(discrimination).lower()}`.", f"Baseline vectors: `{len(baseline_set)}` distinct; final vectors: `{len(pattern_set)}` distinct.", "", "Identical baseline reproduction or identical final vectors require stopping before any repeated sweep."]
