...
_CONTRACTS = {
    ...
    "P3_data_pipeline": """import json, random\nfrom pathlib import Path\ndef checks(root):\n from pipeline.core import load_rows, split_by_group, summarize\n from pipeline.metrics import ordered, report\n rows=load_rows(str(root/'data/measurements.csv')); seed=json.loads((root/'.ekalavya/task.json').read_text())['seed']; random.seed(seed); t1,e1=split_by_group(rows); t2,e2=split_by_group(rows); groups={r['group'] for r in t1},{r['group'] for r in e1}; expected=[('A','2024-01-01'),('A','2024-01-02'),('A','2024-01-03'),('B','2024-01-01'),('B','2024-01-02'),('B','2024-01-03')]\n return [len(rows)==6 and len(t1)+len(e1)==6, [(r['group'],r['timestamp']) for r in ordered(rows)]==expected, groups[0].isdisjoint(groups[1]), t1==t2 and e1==e2, [(r['group'],r['timestamp']) for r in rows]==expected, summarize(rows)['mean']==7.0, summarize(rows)['count']==6 and sum(float(r['value']) for r in rows)==42.0, report(rows)['rows']==rows]\n""",
