...
def _calibration_stop(root):
 p=root/"validation"/"calibration-stop.json"
 if not p.is_file(): return None
 try: return json.loads(p.read_text())
 except (OSError,ValueError): return None
def _vector_label(vector):
 return "".join("P" if x else "F" for x in vector) if vector else "—"
...
def report(root=None):
 root=(root or state_root()).resolve(); rows=_read_rows(root); cal=_read_rows(root,PHASE_CALIBRATION); stop=_calibration_stop(root); allrows=cal+rows
 if stop is not None:
  allrows.append({"resolved":{"provider_model_id":CALIBRATION_CONFIG[0],"reasoning":CALIBRATION_CONFIG[1]},"phase":PHASE_CALIBRATION,"task":{"family":stop.get("next_task_family","unknown")},"status":stop.get("status","harness_failure"),"baseline_score":None,"baseline_check_vector":None,"final_score":None,"final_check_vector":None,"delta_score":None,"normalized_improvement":None,"full_pass":None,"evaluator_tampering":False,"prohibited_changed_files":[],"wall_seconds":None,"input_tokens":None,"output_tokens":None,"cache_read_tokens":None,"reasoning_tokens":None})
 ...
 with (root/"task-check-matrix.csv").open("w",newline="") as h:
  ...
  for r in allrows:
   v=r["final_check_vector"]; bv=r["baseline_check_vector"]; w.writerow({"model":r["resolved"]["provider_model_id"],"reasoning":r["resolved"]["reasoning"],"phase":r["phase"],"task":r["task"]["family"],"status":r["status"],"baseline_score":r["baseline_score"],"baseline_check_vector":"".join("P" if x else "F" for x in bv) if bv else None,"final_score":r["final_score"],"final_check_vector":"".join("P" if x else "F" for x in v) if v else None,**{f"check_{i}":("P" if x else "F") if v else None for i,x in enumerate(v or [],1)},"passed_checks":sum(v) if v else None,"delta_score":r["delta_score"],"normalized_improvement":r["normalized_improvement"],"full_pass":r["full_pass"],"evaluator_tampering":r["evaluator_tampering"],"prohibited_changed_files":json.dumps(r["prohibited_changed_files"]),"wall_seconds":r["wall_seconds"],"input_tokens":r["input_tokens"],"output_tokens":r["output_tokens"],"cache_read_tokens":r["cache_read_tokens"],"reasoning_tokens":r["reasoning_tokens"]})
 (root/"task-check-matrix.md").write_text("# Public Characterization V2.2 task x check matrix\n\nCalibration rows are explicitly excluded from comparative ranking statistics.\n\n"+"\n".join(f"- {r['phase']} / {r['resolved']['provider_model_id']} / {r['task']['family']}: {r['status']}; baseline {_vector_label(r['baseline_check_vector'])}; final {_vector_label(r['final_check_vector'])}; delta {r['delta_score'] if r['delta_score'] is not None else '—'}" for r in allrows)+"\n")
 configs=[]
 ...
 (root/"calibration-summary.json").write_text(json.dumps({"phase":PHASE_CALIBRATION,"model":CALIBRATION_CONFIG[0],"reasoning":CALIBRATION_CONFIG[1],"attempted":len(allrows)-len(rows),"completed":sum(r["status"]=="completed" for r in allrows if r["phase"]==PHASE_CALIBRATION),"harness_failure":sum(r["status"]=="harness_failure" for r in allrows if r["phase"]==PHASE_CALIBRATION),"stop_reason":stop.get("stop_rule") if stop else None,"useful_intermediate_difficulty":_calibration_useful(cal)},indent=2,sort_keys=True)+"\n")
 ...
 cal_attempts=len(allrows)-len(rows); cal_completed=sum(r["status"]=="completed" for r in allrows if r["phase"]==PHASE_CALIBRATION); cal_failures=sum(r["status"]=="harness_failure" for r in allrows if r["phase"]==PHASE_CALIBRATION)
 lines=[f"# {SUITE_NAME}","",f"Class: {EVALUATION_CLASS}; fixed attempt timeout: {ATTEMPT_TIMEOUT_SECONDS} seconds.","",f"Calibration attempts: {cal_attempts} ({cal_completed} scored completed, {cal_failures} harness failure); comparative attempts: {len(rows)}; calibration excluded from rankings.","","## Comparative configuration summary","","| Model | Quality | Mean delta | Full solves | Completed/attempted | Completion | Mean wall | Input/output/cache/reasoning |","|---|---:|---:|---:|---:|---:|---:|---|"]
 ...
 (root/"AUDIT_REPORT.md").write_text("# Public Characterization V2.2 audit\n\nRoot-cause clusters are controller-side validation metadata, not candidate scoring. Calibration is excluded from comparative statistics.\n\n"+("Calibration passed its intermediate-difficulty gate; comparative execution was authorized." if _calibration_useful(cal) else "Calibration did not demonstrate useful intermediate difficulty; comparative execution was not authorized.")+((f" Stop reason: {stop.get('stop_rule')}." if stop else ""))+"\n")
